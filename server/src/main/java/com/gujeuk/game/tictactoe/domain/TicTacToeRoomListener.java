 ...
 */
public interface TicTacToeRoomListener {
    void send(TicTacToeSeat seat, Map<String, Object> message);

    void broadcast(TicTacToeRoom room, Map<String, Object> message);

    /** 승패가 확정됐을 때 레이팅을 반영하고 변동량을 돌려준다. */
    RatingChange finish(Long winnerId, Long loserId, EndReason reason);

    /** 방을 정리해도 되는 시점에 호출된다. */
    void dispose(TicTacToeRoom room);
