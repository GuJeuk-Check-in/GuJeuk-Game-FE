...
새 게임을 만들 때는 먼저 아래 둘 중 맞는 쪽을 열어본다. 구조·네이밍·정리
책임이 거기 다 나와 있다.

| 게임 종류          | 참조                                                | 상태                         |
| ------------------ | --------------------------------------------------- | ---------------------------- |
| DOM 게임           | [`apps/tic-tac-toe`](../apps/tic-tac-toe/README.md) | 표준 골격 (규칙은 비어 있음) |
| 캔버스 + 물리 게임 | [`apps/alkkagi`](../apps/alkkagi)                   | 물리·입력까지 동작           |

`apps/tic-tac-toe`의 [README](../apps/tic-tac-toe/README.md)가 계층 구조와 각
파일의 책임을 정리해 두었다. 사람이든 AI든 이 앱을 읽고 시작하면 된다.
...
apps/tic-tac-toe/src/
  App.tsx             ← 화면. 규칙을 모른다
  game/useGame.ts     ← 상태. 규칙과 화면을 잇는다
  game/rules.ts       ← 규칙. React도 DOM도 모른다
  game/types.ts       ← 타입 계약
```

캔버스 게임은 상태 계층이 훅 대신 게임 클래스다. 클래스가 자기 상태를 갖고
`onChange`로 스냅샷을 올려보내며, `destroy()`에서 루프와 리스너를 직접 뗀다.

이렇게 두면 규칙만 따로 테스트할 수 있고, 나중에 서버 검증(점수 위조 방지 등)에
그대로 재사용할 수 있다.
