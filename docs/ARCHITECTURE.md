...
`server/`는 게임 백엔드(Spring Boot)다. 게임 하나를 붙일 때 화면과 API는 거의
항상 같이 바뀌는데, 저장소가 갈려 있으면 PR도 갈리고 어느 쪽이 먼저 배포되는지에
따라 깨지는 구간이 생긴다. 한 저장소에 두면 그 변경이 한 PR 안에서 끝난다.

다만 **빌드 도구까지 합치지는 않는다.** `server/`는 Yarn 워크스페이스에 넣지
않고 자체 Gradle 프로젝트로 둔다. Turbo는 JS 패키지 그래프를 도는 도구라
Gradle 빌드를 끼워 넣으면 캐시 무효화 규칙이 서로를 오염시킨다. CI에서도
프론트 잡과 서버 잡을 나란히 돌린다.

```
apps/*, packages/*   Yarn 워크스페이스 + Turbo
server/              Gradle (독립)
```

프론트가 서버를 부르는 통로는 `packages/api` 하나다. 게임 앱이 fetch를 직접
쓰지 않는 것과 같은 이유로, 서버 주소·토큰 처리도 그 한 곳에만 있어야 한다.
