 ...
 * 쏠 수 있는 각도 범위.
 *
 * 아래를 35도로 막는 이유는 바람 때문이다. 낮은 탄도는 궤적 정점 근처에서
 * 과녁을 만나는데, 정점에서는 수직 속도가 0이라 바람이 도달 시각을 바꿔도
 * 착탄 높이가 움직이지 않는다. 30도에서는 바람을 통째로 무시해도 실점이
 * 1점뿐이라 읽을 이유가 없어진다. 35도면 3점이다.
 *
 * 속도를 낮춰 낮은 탄도를 못 닿게 막는 방법도 있는데, 그러면 정중앙에 필요한
 * 세기가 만개 근처로 몰려 당기는 폭이 사라진다. 실제로 그렇게 했다가 유효
 * 구간이 100px 드래그 중 16px까지 좁아졌다. 각도로 막고 속도는 남겨 둔다.
 */
const MIN_ANGLE = (35 * Math.PI) / 180
...
function maxPullPx(view: ViewSize): number {
  return clamp(Math.min(view.width, view.height) * 0.45, 130, 260)
}
/**
 ...
 *
 * 이 값은 "당기는 폭"을 정한다. 정중앙에 필요한 세기가 만개에서 멀수록 위아래로
 * 조절할 여지가 생긴다. 24면 35도에서 정중앙이 0.76이고 점수가 나오는 구간이
 * 만개의 67~92%다. 19까지 낮추면 그 구간이 82~98%로 몰려, 만개 100px 드래그
 * 기준 16px 안에 넣어야 점수가 난다 — 조작이 성립하지 않는다.
 *
 * 낮은 탄도가 바람을 무시하는 문제는 속도가 아니라 MIN_ANGLE로 막는다.
 *
 *   각도  정중앙세기   점수 나는 구간      구간폭
 *    35    0.76      만개의 67~92%      24.4%
 *    40    0.72      만개의 66~83%      17.2%
 *    45    0.70      만개의 65~78%      12.8%
 */
const MAX_ARROW_SPEED = 24
/**
 * 바람 세기 1.0당 매 스텝 가해지는 수평 힘 계수.
 *
 * 40도로 정중앙을 조준(세기 0.72)해 놓고 바람만 바꾸면 점수가 이렇게 된다.
 *
 *   바람  -1   -0.5   0   +0.5   +1
 *   점수   4     7    10    10     9
 *
 * 무시하면 최대 6점을 잃는다. 가장 안전한 35도에서도 3점은 잃는다 — 어느
 * 각도로도 바람을 안 읽고 만점을 낼 수는 없다.
 *
 * 계수만 키우면(0.0002) 높은 탄도가 먼저 운 싸움이 되고, 낮은 탄도는 여전히
 * 무풍이나 다름없다. 낮은 탄도 쪽은 MIN_ANGLE이 막는다.
 */
...
export class ArcheryGame {
  ...
  private handleUp(): void {
    ...
    // 각도는 aimState가 이미 범위 안으로 붙여 준다. 여기서는 당기지 않은 것만 거른다.
  ...
  private aimState(): { angle: number; power: number } | null {
    ...
    return {
      // 범위 밖이라고 발사를 막으면 왜 안 나가는지 알 수 없다. 막는 대신 붙인다.
      // 조준선이 한계에 붙어 서므로 어디까지 되는지가 눈에 보인다.
      angle: clamp(Math.atan2(-dy, dx), MIN_ANGLE, MAX_ANGLE),
