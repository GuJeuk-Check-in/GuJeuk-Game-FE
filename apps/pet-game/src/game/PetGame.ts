...
export function initGame(): () => void {
  const KEY = 'gujuk_proto_v2'
  const store = {
    get: async (k: string) => { const v = localStorage.getItem(k); return v ? { key: k, value: v } : null },
    set: async (k: string, v: string) => { localStorage.setItem(k, v); return { key: k, value: v } },
    delete: async (k: string) => { localStorage.removeItem(k); return { key: k, deleted: true } },
  }
  ...
  async function load() { try { const r = await store.get(KEY); return (r && r.value) ? JSON.parse(r.value) : null } catch(_) { return null } }
  let svT: ReturnType<typeof setTimeout> | null = null
  async function save() { try { if(state) state.lastSeen = Date.now(); await store.set(KEY, JSON.stringify(state)) } catch(_) {} }
  function saveSoon() { if(svT) clearTimeout(svT); svT = setTimeout(save, 400) }
  ...
  function render() {
    ...
    $('dailyInfo').textContent = (state.lastLoginDay===today)?('🎁 오늘 출석 보너스 받았어요 · 연속 '+(state.streak||1)+'일 · 내일 또 만나요!'):'🎁 오늘의 출석 보너스를 받을 수 있어요'
  }
  ...
  const FREE: Record<string,{c:number,xp:number,s:string,a:number}> = {feed:{c:1,xp:5,s:'hunger',a:22},play:{c:2,xp:10,s:'happy',a:24},clean:{c:2,xp:6,s:'clean',a:28},sleep:{c:1,xp:4,s:'energy',a:30}}
  function doFree(t: string) {
    const a=FREE[t];if(state.points<a.c){toast('포인트가 부족해요!');return}
    const before=state[a.s];const mn=Math.min(state.hunger,state.happy,state.clean,state.energy)
    state.points-=a.c;state[a.s]=clamp(state[a.s]+a.a)
    const xp=actionXp(a.xp,before,mn);gainXp(xp)
    moving=false;face.classList.remove('walk');mode='action'
    const wasteful=before>=88
    if(t==='sleep'){fxEl(wasteful?'✋':'💤');setTimeout(()=>{mode='idle';if(wasteful)bub(line('full'),2600);else speak('sleep')},700)}
    else{hop();fxEl(wasteful?'✋':(t==='feed'?'🍖':t==='play'?'🎉':'✨'));
      if(wasteful)bub(line('full'),2600);else if(mn<30)bub(line('lowcond'),3000);else speak(t)
      setTimeout(()=>{if(mode==='action')mode='idle'},650);}
    render();saveSoon()
  }
  function petClick() {
  ...
  function buy(id: string) {
    const it=shopFind(id);if(!it||state.points<it.cost)return toast('포인트가 부족해요!')
    state.points-=it.cost
    if(it.slot){state.owned[it.slot].push(id);state.equipped[it.slot]=id;setSprite();speak('dress');hop();fxEl('✨')}
    else if(SHOP.prop.some((p:any)=>p.id===id)){state.owned.prop.push(id);if(state.placed.indexOf(id)<0)state.placed.push(id);renderProps()}
    else{state.owned.buff.push(id);fxEl('✨');toast(it.ic+' 효과 적용!')}
    render();buildShop();saveSoon()
  }
  function toggleEquip(id: string) { const it=shopFind(id);if(!it)return;state.equipped[it.slot]=(state.equipped[it.slot]===id)?null:id;setSprite();if(state.equipped[it.slot])speak('dress');buildShop();saveSoon() }
  function togglePlace(id: string) { const i=state.placed.indexOf(id);if(i>=0)state.placed.splice(i,1);else state.placed.push(id);renderProps();buildShop();saveSoon() }
  ...
  function dailyAndWelcome() {
    const now=Date.now(),el=now-(state.lastSeen||now),hrs=el/3.6e6
    const dm=hasBuff('slowdecay')?0.5:1
    const decayHrs=Math.max(0,hrs-48)
    ...
    const today=new Date().toDateString();let gotDaily=false
    if(state.lastLoginDay!==today){
      if(state.lastLoginDay){const pv=new Date(state.lastLoginDay),nw=new Date();const dd=Math.round((nw.getTime()-pv.getTime())/86400000);state.streak=(dd===1)?(state.streak||0)+1:1}else state.streak=1
      state.maxStreak=Math.max(state.maxStreak||0,state.streak)
      const bonus=hasBuff('ptboost')?4:3;state.points+=bonus;state.lastLoginDay=today;state.visits=(state.visits||0)+1;gotDaily=true
      setTimeout(()=>toast('🎁 출석 보너스 +'+bonus+'P (연속 '+state.streak+'일)'),500)
    }
    const low=(state.hunger<25||state.happy<22),mins=el/60000;let cat: string|null=null
    if(gotDaily)cat='daily';else if(mins<1.5)cat=null;else if(hrs<3)cat='wbShort';else if(hrs<24)cat='wbMid';else cat=low?'wbSad':'wbLong'
    if(state.stage===0&&!gotDaily)cat=(mins<1.5)?null:'wbShort'
    if(cat)setTimeout(()=>speak(cat!,4000),gotDaily?1400:700)
  }
  ...
  function startGame() {
    ...
    if(!wired){wired=true
      ...
      $('menuBtn').addEventListener('click',()=>{
        ...
        $('menuStats').textContent=(state.nick||'')+' · 방문 '+(state.visits||0)+'회 · 연속 '+(state.streak||0)+'일 · '+STAGES[state.species][state.stage]+' 단계 · 효과 '+bf+'개'
        $('menuModal').classList.add('on')
      })
      $('menuClose').addEventListener('click',()=>$('menuModal').classList.remove('on'))
      $('logoutBtn').addEventListener('click',()=>{$('menuModal').classList.remove('on');showScreen('s-login')})
      $('resetBtn').addEventListener('click',async()=>{
        if(confirm('정말 처음부터 다시 시작할까요? 기록이 사라져요.')){try{await store.delete(KEY)}catch(_){}location.reload()}
      })
  ...
  $('loginBtn').addEventListener('click', () => {
    ...
    state.nick=nickEl.value.trim();state.phone=phoneEl.value.trim()
    if(state.species){saveSoon();startGame()}else{buildPicks();showScreen('s-select')}
  })
  ...
  $('selStart').addEventListener('click',()=>{
    state.species=pickSel;state.petName=($('petname') as HTMLInputElement).value.trim()
    state.stage=0;state.xp=0;state.hunger=72;state.happy=78;state.clean=82;state.energy=72
    state.points=5;state.visits=0;state.streak=0;state.maxStreak=0;state.lastLoginDay=null
    state.owned={hat:[],shirt:[],prop:[],buff:[]};state.equipped={hat:null,shirt:null};state.placed=[]
    save();startGame()
  })

  ;(async()=>{
    const s=await load()
    if(s&&s.nick){state=ensure(s);($('nick') as HTMLInputElement).value=s.nick||'';($('loginPhone') as HTMLInputElement).value=s.phone||''}
    checkLogin()
  })()
